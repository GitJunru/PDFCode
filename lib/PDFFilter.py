...
def decompress(data):
    try:
        ...
        lens = len(decompressor.unused_data)
        if lens >= 32:
            decompressed += '__UnusedData__%d__' % lens + decompressor.unused_data
    ...
    if obj.has_key('/Filter'):
        if isinstance(obj['/Filter'], PDFObj.ArrayObject):
            for i in range(0, len(obj['/Filter'])):
                if obj['/Filter'][i] == '/FlateDecode':
                    ...
                    if obj.has_key('/DecodeParms'):
                        obj[PDFObj.NameObject(
                            '__streamdata__')] = PDFObj.StringObject(
                                FlateDecode(obj['__streamdata__'],
                                            obj['/DecodeParms']))
                    else:
                        obj[PDFObj.NameObject(
                            '__streamdata__')] = PDFObj.StringObject(
                ...
                if obj['/Filter'][i] == '/ASCIIHexDecode':
                    ...
                    obj[PDFObj.NameObject(
                        '__streamdata__')] = PDFObj.StringObject(
                            ASCIIHexDecode(obj['__streamdata__']))
                if obj['/Filter'][i] == '/ASCII85Decode':
                    # ASCII85Decode
                    obj[PDFObj.NameObject(
                        '__streamdata__')] = PDFObj.StringObject(
                            ASCII85Decode(obj['__streamdata__']))
                if obj['/Filter'][i] == '/DCTDecode':
                    # DCTDecode
                    obj[PDFObj.NameObject(
                        '__streamdata__')] = PDFObj.StringObject(
                            JPEGCheck.JPEGCheck(obj['__streamdata__']))
                if obj['/Filter'][i] == '/JBIG2Decode':
                    # JBIG2Decode
                    obj[PDFObj.NameObject(
                        '__streamdata__')] = PDFObj.StringObject(
                            JBIG2Decode(obj['__streamdata__']))
            ...
            if obj['/Filter'] == '/ASCIIHexDecode':
                # ASCIIHexDecode
                obj[PDFObj.NameObject('__streamdata__')] = PDFObj.StringObject(
                    ASCIIHexDecode(obj['__streamdata__']))
