...
#                    print >> output,"%08X-%08X:obj"%(obj.start_pos,obj.end_pos-1),obj.getID(),obj.getGeneration(), "Free Object"
                    ...
                    print >> output, "%08X-%08X:obj" % (
                        obj.start_pos, obj.end_pos -
                        1), obj.getID(), obj.getGeneration(), "Hint Stream"
                elif obj.fXMLmetadata:
                    print >> output, "%08X-%08X:obj" % (
                        obj.start_pos, obj.end_pos -
                        1), obj.getID(), obj.getGeneration(), "XML metadata"
                elif obj.fTrailer:
                    print >> output, "%08X-%08X:obj" % (
                        obj.start_pos, obj.end_pos -
                        1), obj.getID(), obj.getGeneration(), "trailer"
                elif obj.fXrefStm:
                    print >> output, "%08X-%08X:obj" % (
                        obj.start_pos, obj.end_pos -
                        1), obj.getID(), obj.getGeneration(), "XrefStm"
                elif obj.fObjStm:
                    print >> output, "%08X-%08X:obj" % (
                        obj.start_pos, obj.end_pos - 1), obj.getID(
                        ), obj.getGeneration(), "ObjStm", obj.ObjStm
                else:
                    if obj.xref is None and obj.end_pos - obj.start_pos > 512:
                        # Suspicious
                        print >> output, "%08X-%08X:obj" % (
                            obj.start_pos,
                            obj.end_pos - 1), obj.getID(), obj.getGeneration(
                            ), "xref from", obj.xref, "Suspicious"
                    else:
                        print >> output, "%08X-%08X:obj" % (
                            obj.start_pos, obj.end_pos - 1), obj.getID(
                ...
                unfinish = True
                print >> output, "%08X-%08X:xref" % (obj.start_pos,
                                                     obj.end_pos - 1)
            elif obj.fStartXref:
                unfinish = True
                print >> output, "%08X-%08X:startxref %08X" % (
                    obj.start_pos, obj.end_pos - 1, obj)
            elif obj.fComment:
                if obj.fEOF:
                    print >> output, "%08X-%08X:EOF," % (
                        obj.start_pos, obj.end_pos - 1)  # ,obj
                    eof += 1
                    unfinish = False
                else:
                    print >> output, "%08X-%08X:comment," % (
                        obj.start_pos, obj.end_pos - 1)  # ,obj
            elif obj.fEtc:
                if obj.end_pos - obj.start_pos + 1 > 512:
                    print >> output, "%08X-%08X:unknown(malicious)" % (
                        obj.start_pos, obj.end_pos - 1)
                else:
                    print >> output, "%08X-%08X:unknown(suspicious)" % (
                        obj.start_pos, obj.end_pos - 1)
            else:
                if obj.end_pos - obj.start_pos + 1 > 512:
                    print >> output, "%08X-%08X:unknown(malicious)" % (
    ...
    def ReadFile(self, filename, password=''):
        ...
        while not self.stream.isEOF():
            ...
            if m1 or m1_:
                ...
                if m1:
                    # obj签的两个数字，第一个为idnum，第二个为generation
...
# self.stream.skip_blank()
                    ...
                    if isinstance(obj, PDFObj.DictionaryObject):
                        if obj.has_key('/Length'):
                            lens = obj['/Length']
                    if isinstance(lens, PDFObj.IndirectObject):
                        if self.isObj2(lens.getID()):
                            lens = self.GetObj2(lens.getID())
                            if not isinstance(lens, PDFObj.NumberObject):
                                lens = 0
                        else:
                            lens = 0
                    pos = self.stream.get_pos()
                    ...
                    while tok != 'EOF':
                        ...
                        tok = self.stream.get_u()
                        self.stream.bak_pos()
                    if lens != 0 and len(txt) > lens and len(
                            txt) > 2 and txt[-1] in ('\r', '\n'):
                        if len(txt) > lens + 1 and len(txt) > 3 and txt[-2] in (
                                '\r', '\n'):
                            txt = txt[:-2]
                        else:
                            txt = txt[:-1]
                ...
                for i in range(len(line)):
                    if line[i] == '\x00':
                        lens = i + 1
                        break
                self.stream.set_pos(start_pos + lens)
                end_pos = self.stream.get_pos()
                if line[-1] in ('\r', '\n'):
                    if line[-2] in ('\r', '\n'):
                        line = line[:-2]
        ...
        if t_encrypt:
            ...
            if not encrypt.has_key('/EncryptMetadata'):
                encrypt[PDFObj.NameObject(
                    '/EncryptMetadata')] = PDFObj.BooleanObject(True)
            if not encrypt.has_key('/Length'):
                encrypt[PDFObj.NameObject('/Length')] = PDFObj.NumberObject(40)
            self.encrypt = encrypt
            encryption_key = PDFCrypto.get_encryption_key(
                self.encrypt, self.Trailer, password)
                            ...
                            if obj['/Type'] == '/XRef':
                                ...
                                if not obj.fTrailer:
                                    if self.Trailer.has_key('/XRefStm'):
                                        if obj.start_pos != int(
                                                self.Trailer['/XRefStm']):
                                            fXref = False
                                    else:
                                        fXref = False

                    if (obj.getID() != encrypt.getID() or obj.getGeneration !=
                            encrypt.getGeneration()) and not fXref:
                        PDFCrypto.DecryptObject(encrypt, encryption_key, obj,
                                                obj.getID(),
                                                obj.getGeneration())
        ...
        for obj in self.obj:
            if obj.fOBJ:
                if isinstance(obj, PDFObj.DictionaryObject) and obj.has_key(
                        '/Linearized'):
                    if obj['/Linearized'] == 1:
                        # Linear PDF
                        obj.fLinearPDF = True
                ...
                for var in xref:
                    if self.isObj2(var.getID()):
                        ...
                        if not obj2.xref:
                            obj2.xref = []
                        obj2.xref.append(
                            PDFObj.IndirectObject(obj.getID(),
                                                  obj.getGeneration()))
            elif obj.fTrailer:
                xref = SearchIndirectObject(obj)
                for var in xref:
